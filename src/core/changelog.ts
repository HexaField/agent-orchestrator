...
export async function writeChangelog(
  ...
  try {
    ...
    const runs = (await (await import('fs/promises')).readdir(runsDir))
    ...
    if (last) {
      const runJson = JSON.parse(
        ...
        ).readFile(path.join(runsDir, last, 'run.json'), 'utf8'),
      );
      ...
      if (gf) {
        ...
        const small = gf.diff
          ? String(gf.diff).slice(0, 8000) +
            (String(gf.diff).length > 8000 ? '\n...(truncated)' : '')
          : '';
        verification += `\n\n## Git changes\n${files}\n\n${diffPreview}\n\n${small}`;
      }
    }
