...
export async function runVerification(): Promise<VerificationResult> {
  if (
    ...
    return {
      lint: 'pass',
      typecheck: 'pass',
      tests: { passed: 1, failed: 0, coverage: 0 },
      custom: [],
    };
  }
  let pkg: any = {};
  ...
  const lint = hasScript('lint')
    ? await tryRun('npm', ['run', '-s', 'lint'])
    : { ok: true };
  const typecheck = hasScript('typecheck')
    ? await tryRun('npm', ['run', '-s', 'typecheck'])
    : { ok: true };
  const tests = hasScript('test')
    ? await tryRun('npm', ['run', '-s', 'test'])
    : { ok: true };
  return {
