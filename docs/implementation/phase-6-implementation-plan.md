...
Phase 5 delivers the human-in-the-loop review workflow and first-class VCS integration. It implements a Review Gate that iterates through tasks, and provides safe, auditable git commit/branch/PR adapters. This phase closes the loop from verified artifacts to repository commits and human approval.
...
- Changelog & PR description generation from provenance and run tracking artifacts
...
Define and implement a deterministic run-tracking format that records the run's tasks, status, provenance pointers, and reviewer decisions. The Review Gate service must iterate run-tracking tasks and enforce that no task marked as `applied` reaches `commit` before reviewer approval (except when `--auto-commit` is explicitly allowed and enabled by policy).
...
- Generate human-friendly changelogs and PR descriptions from the run's provenance and run-tracking entries.
...
Purpose: provide a single-source-of-truth for run progress, tasks, and review state that can be read and updated by orchestrator components, reviewers, and verifier agents. Runs must persist a compact run-tracking artifact under `.agent/run/<id>/` and append an immutable log for auditability.
...
- The orchestrator should cap run-tracking size or shard large runs into multiple files if a run grows very large.
...
- The Review Gate reads the run-tracking artifact and exposes an ordered list of tasks that reached `applied` and `verified` states but are `unreviewed`.
...
1. Orchestrator writes tasks to the run-tracking artifact as they are applied and verified.
2. A blocking call to the Review Gate (CLI or API) lists unreviewed tasks grouped by changelist.
3. The human reviewer walks the list, acting on each item. The review client writes back the reviewer decision into the run-tracking artifact and `progress.log`.
...
Purpose: make it safe and reproducible to turn approved run-tracking tasks into commits, branches, and PRs.
...
- Use run-tracking artifacts + `.agent/run/<id>/provenance/*.json` to render:
...
1. After TaskLoop finishes a run, produce the run-tracking artifact and `report.md`.
...
- Unit tests:
   - run-tracking artifact writer/reader: concurrent writers, corruption resistance, and schema validation.
...
- Run-tracking artifacts exist for runs and follow the chosen schema; updates are atomic and logged to `progress.log`.
...
- Wire provider webhooks (PR events) to automatically update the run-tracking artifact when reviewers act on the PR in the provider UI.
...
- Create a simple CLI `review-client` that reads the run-tracking artifact and allows approve/request-changes from the terminal.
